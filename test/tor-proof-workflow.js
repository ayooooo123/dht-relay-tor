...
test('Tor proof workflow locks its supply-chain inputs', (t) => {
  ...
  t.ok(/^\s*BARE_ARTI_SHA:\s*e00c6c0d2658a65d833a4b08a510096408d7edb0\s*$/m.test(workflow))
...
test('Tor proof workflow separates the control and embedded proof', (t) => {
  ...
  const systemJob = jobBlock(workflow, 'system_tor')
  t.absent(/reachableAddresses|assemble-proof-package|bare-arti/.test(systemJob))
})
...
test('embedded proof rebuilds, packs, and verifies the exact bare-arti checkout', (t) => {
  ...
  t.ok(/node scripts\/assemble-proof-package\.js/.test(workflow))
  t.ok(/npm pack --ignore-scripts --json/.test(workflow))
  t.ok(/p\.private!==true/.test(workflow), 'verifies the proof package remains private')
  t.ok(/v\.proofOnly!==true/.test(workflow), 'labels the ephemeral proof provenance')
  ...
  t.ok(/GITHUB_RUN_ID/.test(workflow))
  t.ok(/GITHUB_RUN_ATTEMPT/.test(workflow))
